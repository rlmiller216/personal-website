...
│           ├── notion-block-utils.ts     # Shared helpers: extractRichText, extractMediaUrl, groupListItems, parseWidthDirective (~110 LOC)
...
│           ├── image-cache.ts           # Build-time Notion file downloader: images + videos → static/images/, PDFs/docs/.txt/.md → static/files/, HEIC→JPEG conversion, dedup, hash. downloadItemMedia (renamed from downloadItemImages) (~148 LOC)
...
- `notion-block-utils.ts` — `extractRichText()`, `extractMediaUrl()`, `createBaseBlock()`, `groupListItems()`, `parseWidthDirective()`
...
- `image-cache.ts` — `downloadNotionImage()`, `downloadNotionFile()`, `downloadItemMedia()` (renamed from `downloadItemImages`), `isNotionS3Url()`, `hashUrlToFilename(url, defaultExt = '.jpg')`. Downloads Notion S3 images and videos (mp4/webm/mov) to `static/images/` and files (PDFs, `.txt`, `.md`, etc.) to `static/files/` at build time. Auto-detects HEIC images (iPhone uploads) via magic bytes and converts to JPEG via `heic-convert`. Shared `downloadS3File()` internal with content-type safelist (includes `video/*`, `text/plain`, `text/markdown`). Unknown extensions on the file path default to `.bin` (image path keeps `.jpg`). Deduplicates via promise Map, falls back to original URL on failure.
