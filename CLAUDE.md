...
### Key Patterns
- `overflow-x: hidden` on `body` (NOT `html`) — prevents horizontal bounce on mobile from elements slightly exceeding viewport width. **Never put this on `<html>`:** per CSS spec, `overflow-x` on the root promotes `<html>` to a scroll container (its `overflow-y` silently becomes `auto`), which breaks `position: sticky` on descendants. On iOS Safari the break is scroll-direction-asymmetric — sticky headers scroll away on scroll-down while the URL-bar retracts, then snap back on scroll-up when the chrome returns.
